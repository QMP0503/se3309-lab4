            ...
            <div className='create-page-wrapper'>
                <div className='create-selects'>
                    ...
                    <label>
                        ...
                        <select className='drop-down' onChange={e => {
                            const selectedGem = JSON.parse(e.target.value);
                            setGem(selectedGem)
                        }}>
                    ...
                    <label>
                        Metal:
                        <select className='drop-down' onChange={e => {
                            const selectedMetal = JSON.parse(e.target.value);
                ...
                {type === "necklace" &&
                    <div className='type-customization'>
                        <h2>Customize Necklace</h2>

                        <label>
                            Link Type:
                            <select className='drop-down' onChange={e => {
                                const selectedLink = JSON.parse(e.target.value);
                        ...
                        <label>
                            Link Amount:
                            <input className="drop-down" onKeyDown={event => event.preventDefault()} type='number'
                                   min={20} max={100} defaultValue={20}
                                   onChange={e => setLinkNum(e.target.value)}></input>
                        </label>

                        <label>
                            Name:
                            <input type='text' placeholder='...' className='name-input'
                                   onChange={e => setNeckName(e.target.value)}></input>
                        </label>
                ...
                {type == "ring" &&
                    <div className='type-customization'>
                        ...
                        <label>
                            ...
                            <input className="drop-down" type='number' min={3} max={16} step={0.5} defaultValue={3}
                                   onChange={e => setRingSize(e.target.value)}/>
                        </label>

                        <label>
                            Volume {"(mm^3)"}:
                            <input className="drop-down" type='number' min={1} max={100} step={0.1} defaultValue={20}
                                   onChange={e => setRingVolume(e.target.value)}></input>
                        </label>

                        <label>
                            Name:
                            <input type='text' placeholder='...' className='name-input'
                                   onChange={e => setRingName(e.target.value)}></input>
                        </label>
    ...
    function createNecklaceOrder() {
        if (!sessionStorage.getItem("user")) {
            ...
            fetch("/api/products", {
                ...
                body:{
                    name: neckName,
                    mass: link.volume * link.size * linkNum + metal.density,  //#links * link vol * metal density
                    price: metal.price + gems.price, //mass*$pg metal + gem
